...
namespace Stock_Web_Api.Consumers
{
	public class OrderCreatedEventConsumer : IConsumer<OrderCreatedEvent>
	{
		...
		public  async Task Consume(ConsumeContext<OrderCreatedEvent> context)
		{
            ...
            {
				stockResult.Add((await _stockCollection.FindAsync(s => s.ProductId == orderItem.ProductId && s.Count
				>= orderItem.Count)).Any());
            }
			if(stockResult.TrueForAll(sr => sr.Equals(true)))
			{
                foreach (OrderItemMessage orderItem in context.Message.OrderItems)
                {
				  Stock stock =	await(await _stockCollection.FindAsync(s => 
				  s.ProductId == orderItem.ProductId)).FirstOrDefaultAsync();
				...
				await Console.Out.WriteLineAsync("Stock operation successfull");
            ...
            } else
			{
				...
				await Console.Out.WriteLineAsync("Stock operation Failed");
			}
